...
with open('drowsiness_events.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Timestamp", "Event"])  # Write header
    ...
    for face in faces:
        ...
        if ear < EAR_THRESHOLD:
            ...
            if frame_count_ear >= CONSEC_FRAMES_EAR and not alarm_on_ear:
                ...
                with open('drowsiness_events.csv', mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([time.strftime("%Y-%m-%d %H:%M:%S"), "Eyes Closed"])
        else:
            frame_count_ear = 0
            alarm_on_ear = False
        ...
        if mar > MAR_THRESHOLD:
            ...
            if frame_count_mar >= CONSEC_FRAMES_MAR and not alarm_on_mar:
                ...
                with open('drowsiness_events.csv', mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([time.strftime("%Y-%m-%d %H:%M:%S"), "Yawning"])
        else:
            frame_count_mar = 0
            alarm_on_mar = False
        ...
        if abs(head_tilt_angle) > HEAD_TILT_THRESHOLD:
            ...
            if frame_count_head >= CONSEC_FRAMES_EAR and not alarm_on_head:
                ...
                with open('drowsiness_events.csv', mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([time.strftime("%Y-%m-%d %H:%M:%S"), "Head Tilt"])
        else:
            frame_count_head = 0
            alarm_on_head = False
